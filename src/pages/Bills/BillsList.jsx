              ...
              <tr key={b.id} className="hover:bg-gray-50">
                ...
                <td className="px-5 py-3">
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => setView(b)} className="btn-small bg-white border border-gray-200 text-gray-400 hover:text-blue-600" aria-label={`View ${b.number}`}><Eye size={14} /></button>
                    {b.status === 'open' && (
                      <button onClick={() => updateBill(b.id, { status: 'paid' })} className="btn-small bg-blue-50 border border-blue-200 text-blue-700 px-2" aria-label={`Mark ${b.number} paid`}>Mark Paid</button>
                    )}
                    <button onClick={() => openEdit(b)} className="btn-small bg-white border border-gray-200 text-gray-400 hover:text-blue-600" aria-label={`Edit ${b.number}`}><Pencil size={14} /></button>
                    <button onClick={() => deleteBill(b.id)} className="btn-small bg-white border border-gray-200 text-gray-400 hover:text-red-600" aria-label={`Delete ${b.number}`}><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
